...
const skillsData = [
  { name: "Frontend Architecture", items: ["React.js", "Next.js", "TypeScript", "Tailwind CSS", "Framer Motion"], color: "from-primary to-secondary", icon: "01" },
      ...
      <div className="max-container">
            ...
            >
              <SpotlightCard className="p-1 h-full">
                <div className="bg-black/90 p-10 rounded-lg h-full border border-white/5 group">
                  ...
                  <h3 className={`text-2xl font-black mb-8 tracking-tighter uppercase text-white group-hover:text-primary transition-colors`}>{category.name}</h3>
                  <div className="flex flex-wrap gap-3">
                    {category.items.map(item => (
                      <span key={item} className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-xs font-bold text-gray-400 hover:text-primary hover:border-primary/30 transition-all cursor-default">
                        {item}
                      </span>
                    ))}
                  </div>
        ...
        <div className="mt-24 text-center">
           <BorderGlow className="inline-block p-1 rounded-xl">
              <a href={siteConfig.resumeLink} className="px-16 py-5 bg-black rounded-lg text-lg font-black uppercase tracking-widest text-white hover:text-primary transition-colors">
                Download Manuscript
              </a>
           </BorderGlow>
        </div>
