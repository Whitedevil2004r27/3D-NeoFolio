    ...
    <div className="fixed bottom-10 right-10 z-[200]">
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="p-6 bg-gradient-to-r from-cyan-400/10 to-purple-600/10 border-b border-white/5 flex justify-between items-center">
            ...
            <div className="h-80 overflow-y-auto p-6 space-y-4 scrollbar-hide">
              {messages.map((m, i) => (
                ...
                >
                  <div className={`max-w-[80%] p-4 rounded-xl text-[10px] md:text-xs font-mono tracking-tight leading-relaxed ${m.role === 'user' ? 'bg-cyan-400/10 border border-cyan-400/20 text-white' : 'bg-white/5 border border-white/10 text-gray-400'}`}>
                    {m.content}
                  </div>
                </motion.div>
              ))}
            ...
            <div className="p-4 border-t border-white/5 bg-black/50 flex gap-2">
              <input 
                ...
                className="flex-1 bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-[10px] font-mono text-white outline-none focus:border-cyan-400/50 transition-all placeholder:text-gray-700"
              />
              <button 
                onClick={handleSend}
                className="p-3 bg-cyan-400 text-black rounded-lg hover:bg-cyan-300 transition-colors"
              >
                <Send size={18} />
              </button>
            </div>
      ...
      >
        <div className="absolute -inset-4 bg-cyan-400 rounded-full blur-2xl opacity-20 group-hover:opacity-40 transition-opacity" />
        <div className="w-16 h-16 md:w-20 md:h-20 bg-black/80 backdrop-blur-xl border border-cyan-400/30 rounded-full overflow-hidden shadow-[0_0_30px_rgba(0,243,255,0.2)]">
        ...
        <div className="absolute -top-1 -right-1 w-5 h-5 bg-cyan-400 rounded-full border-4 border-black flex items-center justify-center">
