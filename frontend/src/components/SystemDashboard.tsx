...
export const SystemDashboard = () => {
  ...
  const metrics = [
    { label: "Core Uptime", value: `${data.uptime}s`, icon: Zap, color: "text-primary" },
        ...
        <SpotlightCard className="p-1">
          <div className="bg-black/90 p-8 md:p-12 rounded-xl border border-white/5 relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
              ...
              <div className="px-4 py-2 bg-primary/5 border border-primary/20 rounded-full flex items-center gap-3">
                <div className="w-2 h-2 bg-primary rounded-full animate-pulse shadow-[0_0_10px_var(--primary)]" />
                <span className="text-[10px] font-black text-primary uppercase tracking-widest">Live Sync Active</span>
              </div>
            </div>
                ...
                >
                  ...
                  <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                    <motion.div 
                      ...
                      className={`h-full bg-gradient-to-r ${m.color === 'text-primary' ? 'from-primary/50' : m.color.replace('text-', 'from-').replace('-400', '-400/50')} to-transparent`}
                    />
                  </div>
                </motion.div>
              ))}
